...
# get user input
link = input("Enter a YouTube link you would like to download: \n")
yt = YouTube(link)
...
for stream in yt.streams.filter(adaptive=True, audio_codec = 'mp4a.40.2'):
    ...
    stream.download('C:/Users/phamh', filename='audio.mp4')
...
for stream in yt.streams.filter(adaptive=True, file_extension='mp4'):
    ...
    if stream.resolution != res:
        print(stream.resolution)
    res = stream.resolution
    stream.download('C:/Users/phamh', filename='video.mp4')
